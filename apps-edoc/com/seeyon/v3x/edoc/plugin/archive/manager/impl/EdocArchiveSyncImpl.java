...
public class EdocArchiveSyncImpl implements IArchiveSync {
            ...
            for(Long id : ids) {
                ...
                try {
                    ...
                    if(summary != null){
                        ...
                        if(state != CollaborationEnum.flowState.finish.ordinal() && state != CollaborationEnum.flowState.terminate.ordinal()){
    ...
    private String analysisFormContent(EdocSummary summary, ArchiveDocBO vo) throws BusinessException{
        ...
        //传入公文类型
        field2ValueMap.put("edoc_type", new String[]{String.valueOf(summary.getEdocType())});
        //解析字段
    ...
    private void setExtendInfo(Map<String, String[]> field2ValueMap, ArchiveDocBO vo) {
    	...
    	vo.addExtDesc("edoc_type", field2ValueMap.get("edoc_type") == null ? "" : field2ValueMap.get("edoc_type")[0]); // 公文类型
