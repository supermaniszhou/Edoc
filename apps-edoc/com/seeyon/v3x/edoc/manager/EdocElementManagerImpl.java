...
public class EdocElementManagerImpl implements EdocElementManager {
    ...
    private Integer getAllEdocElementCount(String condition, String textfield) {
    	List<EdocElement> list = new ArrayList<EdocElement>();
    	list = edocElementCacheManager.getEdocElementCacheListByDomainId(AppContext.currentAccountId());
    	int elementCount = 0;
    	if (Strings.isNotBlank(textfield)) {
	    	...
	    	if ("elementName".equals(condition)) {
	    		...
	    		{
	    			EdocElement element = (EdocElement)list.get(i);
	    			//元素名称
	    			String name = element.getName();
	    			//登记改为分发
	    			if(EdocHelper.isG6Version() && element.getIsSystem() && "edoc.element.dengji".equals(name)){
	    				name = "edoc.element.fenfa";
	    			}
