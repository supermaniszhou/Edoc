...
*/	
	...
	private static EdocObjTeamManager edocObjTeamManager = (EdocObjTeamManager)AppContext.getBean("edocObjTeamManager");
...
//		导出excel文件的国际化
	...
	public static void generateZipperFleet(long accountId){
		...
		generateZipperFleet(accountId,false);
	}
	
	public static void generateZipperFleet(long accountId,boolean newPlugin){
		...
		if(newPlugin){//OA-149505升级前无公文模块，升级后公文节点权限对所有公文元素都是只读的
			generateEdocElementFlowPermAclByAccountId(accountId);
		}
	}
	
	
	private static void generateEdocElementFlowPermAclByAccountId(long accountId){
		LOGGER.info("开始为新建单位复制公文节点权限绑定的公文元素状态...accountId="+accountId);
		try{
		    ...
		    //查询发文
		    edocList = configManager.listAllConfigByCategory(EnumNameEnum.edoc_send_permission_policy.name(), accountId);
		    if(Strings.isNotEmpty(edocList)){
		    	LOGGER.info("发文节点权限不为空");
		    	list.addAll(edocList);
		    }
		    //查询收文
            edocList = configManager.listAllConfigByCategory(EnumNameEnum.edoc_rec_permission_policy.name(), accountId);
            if(Strings.isNotEmpty(edocList)){
            	LOGGER.info("收文节点权限不为空");
		    	list.addAll(edocList);
		    }
            //查询 签报
            edocList = configManager.listAllConfigByCategory(EnumNameEnum.edoc_qianbao_permission_policy.name(), accountId);
            if(Strings.isNotEmpty(edocList)){
            	LOGGER.info("签报节点权限不为空");
		    	list.addAll(edocList);
		    }
            if(Strings.isNotEmpty(list)){
            	Map<Long,ConfigItem> emap = new HashMap<Long,ConfigItem>();
            	for(ConfigItem item:list){
                     emap.put(item.getId(), item);
            	}
            	saveEdocElementPermByAccountId(emap);
		    }
		}catch(Exception e){
			LOGGER.error("新建单位的时候复制公文节点权限绑定的公文元素状态异常",e);
		}
		LOGGER.info("复制公文节点权限绑定的公文元素状态结束。");
	}
	
	private static void saveEdocElementPermByAccountId(Map<Long, ConfigItem> arg0) throws BusinessException {
	...
	public static void generateEdocFormByAccountId(long accountId){
		...
		try{
            ...
            if(Strings.isNotEmpty(thisForms)){
                ...
                for (EdocForm oldForm : thisForms) {
                    if(oldForm.getIsSystem()){
                      //有系统公文单了就不再进行修复了
                        LOGGER.info("存在系统公文单...");
                        return;
                    }
			...
			for(EdocForm form : accountList){
				...
				if(user == null){
					newForm.setCreateUserId(-8060678314709529794l);//系统预制的ID，此处保持与初始化相同
				}else{
			...
			LOGGER.error("为新建单位复制公文单失败"+e.getLocalizedMessage(),e);
		}
		LOGGER.info("公文单复制完毕");
	}
	...
	public static String getName(String ids,String seperator) throws Exception{
		 ...
		 for(String sid : sids){
			 ...
			 try {
				 String[] typeAndId = sid.split("[|]");
				 if("OrgTeam".equals(typeAndId[0])){
					 EdocObjTeam team = edocObjTeamManager.getById(Long.valueOf(typeAndId[1]));
					 if(team!=null){
						 name.append(team.getName());
					 }
				 }else{
					 V3xOrgEntity  entity = orgManager.getEntity(sid);
					 if(entity!=null){
						 name.append(entity.getName());
					 }
				 }
			} catch (Exception e) {
				LOGGER.error("获取组织机构异常:",e);
			}
		 }
	    ...
	    public static String getErrorMsgByAffair(CtpAffair affair)
	    {
	        ...
	        if(affair != null) {
	            ...
	            if(Strings.isNotBlank(state)){
	                msg = ResourceUtil.getString("collaboration.state.invalidation.alert", StringEscapeUtils.escapeJavaScript(affair.getSubject()), state,appName, forwardMemberFlag, forwardMember);
	            }
	        }else{
	            state = ResourceUtil.getString("collaboration.state.9.delete");
...
//          affairData.setResentTime(summary.getResentTime());//如协同colsummary
	...
	//      }
	    ...
	    public static String getAttachments(List<Attachment> attachmentList) {
	        ...
	        for(int i = 0;i<attachmentList.size();i++) {
				...
				attachments.append((i+1)).append(".").append(filename);
            }
	        return attachments.toString();
	    }
	...
	public static void updateCurrentNodesInfo(EdocSummary summary,boolean isUpdateToDb) throws Exception{
		...
		List<CtpAffair> affairs = affairManager.getAffairsForCurrentUsers(fi, map);
