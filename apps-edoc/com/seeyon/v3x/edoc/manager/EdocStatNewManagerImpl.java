...
import com.seeyon.v3x.edoc.webmodel.EdocStatListResultVO;
...
public class EdocStatNewManagerImpl implements EdocStatNewManager {
	...
	public FlipInfo getEdocStatVoList(FlipInfo flipInfo, Map<String, String> conditionMap) throws BusinessException {
		...
		if(Strings.isNotBlank(conditionMap.get("rangeIds"))) {
			...
			List<EdocStatListResultVO> resultList = new ArrayList<EdocStatListResultVO>();
			for(EdocStatVO vo : statVoList){
				EdocStatListResultVO resultVo = new EdocStatListResultVO(vo);
				...
				for(Object[] object : result) {
					...
					if(statVoMap.containsKey(displayId)) {
						...
						if(resultType == EdocStatEnum.EdocStatResultTypeEnum.pendingAndDone.key()) {	//待办/已办/待阅/已阅
							...
							if(resultVo.getDisplayRow().equals(String.valueOf(CollaborationEnum.flowState.finish.ordinal()))
									|| resultVo.getDisplayRow().equals(String.valueOf(CollaborationEnum.flowState.terminate.ordinal()))) {
								statVo.setCountFinish(resultVo.getDisplayRowCount());
							} else if(resultVo.getDisplayRow().equals(String.valueOf(CollaborationEnum.flowState.run.ordinal()))) {
	...
	private void setEdocStatVoListBak(FlipInfo flipInfo, Map<String, String> conditionMap, Map<String, Object> statVoMap) throws BusinessException {
    	...
    	boolean isCurrentAccountExchange = EdocRoleHelper.isAccountExchange();
		boolean isCurrentDeptExchange = EdocRoleHelper.isDepartmentExchange();
		boolean showBanwenYuewen = EdocSwitchHelper.showBanwenYuewen(user.getLoginAccount());
	...
	public FlipInfo getEdocVoList(FlipInfo flipInfo, Map<String, String> conditionMap) throws BusinessException {
		...
		finishStatList.add(CollaborationEnum.flowState.finish.ordinal());
		finishStatList.add(CollaborationEnum.flowState.terminate.ordinal());
		...
		if(Strings.isNotEmpty(result)) {
			...
			for(Object[] object : result) {
	            ...
	            if(listVo.getDeadlineDatetime()==null) {
	            	listVo.setDeadlineTimeView((deadline == null || deadline == 0) ? isNull : ColUtil.getDeadLineNameForEdoc(deadline, listVo.getStartTime()));
	            } else {
...
//		boolean isCurrentDeptExchange = EdocRoleHelper.isDepartmentExchange();
			...
			for(int i=0; i < ranges.length; i++) {
				...
				if(!deptMap.containsKey(deptId)) {
					...
					if(!isCurrentAccountExchange) {
						...
						isDeptExchange = EdocRoleHelper.isDepartmentExchange(user.getLoginAccount(), deptId, user.getId());
			...
			for(int i=0; i<ranges.length; i++) {
				...
				if("Department".equals(rangeType)) {
					...
					if(Strings.isNotEmpty(memberList)) {
					    ...
					    boolean isDeptExchange = false;
                        if(!isCurrentAccountExchange) {
                            isDeptExchange = EdocRoleHelper.isDepartmentExchange(user.getLoginAccount(), rangeId, user.getId());
                            if(!isDeptExchange) {
	...
	private EdocStatVO getCountEdocStatVO(EdocStatVO statVo, EdocStatParamVO paramVo, boolean showBanwenYuewen, Map<String, Long> statAffairMap) {
		...
		Boolean isFinish = state == CollaborationEnum.flowState.finish.ordinal() || state == CollaborationEnum.flowState.terminate.ordinal();
		String keyDisplayId = "";
		...
		if(paramVo.getEdocType().intValue() == 1) {//收文
			...
			if(isDo && state!=CollaborationEnum.flowState.terminate.ordinal() 
