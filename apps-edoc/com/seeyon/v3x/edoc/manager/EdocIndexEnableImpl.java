...
public class EdocIndexEnableImpl extends EdocManagerImpl implements IndexEnable {
	...
	@Override
	public Integer findIndexResumeCount(Date arg0, Date arg1)
			throws BusinessException { 
		   StringBuffer sbHql = new StringBuffer("select count(b.id) from EdocSummary ");
	        sbHql.append(" as b where b.createTime >= ? and b.createTime <= ? ");
	        Number result = 0;
		...
		if (Strings.isNotEmpty(affairs)){
			...
			if(Long.valueOf(0l).equals(userId)){
				...
				for(CtpAffair affair : affairs){
					...
					if(affair.getMemberId().equals(userId) && isEdocIndexenable(affair.getApp()) && ColUtil.isAfffairValid(affair, false)){
	...
	public IndexInfo getIndexInfo(Long id) throws BusinessException {
		...
		info.setEntityID(id);
		info.setAppType(EdocUtil.getAppCategoryByEdocType(ApplicationCategoryEnum.edoc.getKey()));
