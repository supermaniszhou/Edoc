...
public class EdocFormManagerImpl implements EdocFormManager {
    ...
    public void createEdocForm(EdocForm edocForm, List<Long> elementIdList) {
        ...
        if (eleNum == 4) {
            edocForm.setIsunit(true);
        } else {
            edocForm.setIsunit(false);
        }
 ...
 * 	  private EdocForm _getDefaultEdocForm(Long domainId,String domainIds,int edocType) {
    ...
    public Hashtable<String, String> getOpinionLocation(Long edocFormId, Long accountId) {
        ...
        for (EdocFormFlowPermBound eb : ls) {
            ...
            if (eb.getFlowPermName() != null) {
                hs.put(eb.getFlowPermName(), eb.getProcessName() + "_" + eb.getSortType());
                if ("zhihui".equals(eb.getFlowPermName()) || "inform".equals(eb.getFlowPermName())) {
                    hs.put("zhihui", eb.getProcessName() + "_" + eb.getSortType());
                    hs.put("inform", eb.getProcessName() + "_" + eb.getSortType());
                }
            }
        }
        return hs;
     ...
     * @return
     */
    public List<WorkflowFormFieldBO> getElementByEdocForm(Long formId) {

        EdocForm edocForm = getEdocForm(formId);
        List<WorkflowFormFieldBO> tfList = new ArrayList<WorkflowFormFieldBO>();
        ...
        for (EdocFormElement efe : eles) {
            try {
                WorkflowFormFieldBO tf = new WorkflowFormFieldBO();
    ...
    public Object[] getEdocFormElementRequiredMsg(EdocForm edocForm, EdocSummary summary) throws Exception {
        ...
        StringBuffer notPassMsg = new StringBuffer();
     ...
     */
    @Override
    public boolean writeFormFileZlc(Long summaryId, String folder) {
        return FormParseUtil.writeFormFileZlc(summaryId, folder);
    }
    ...
    @Override
    public List<WorkflowFormFieldBO> getAllElementsByEdocForm(Long formId) {
        EdocForm edocForm = getEdocForm(formId);
        List<WorkflowFormFieldBO> tfList = new ArrayList<WorkflowFormFieldBO>();
        Set<EdocFormElement> eles = null;
        try {
