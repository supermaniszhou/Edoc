 ...
 *
 */
public class EdocMarkReserveCacheManager {
	...
	public EdocMarkReserveVO getCacheEdocMarkReserveVoById(Long reserveId) throws BusinessException {
		reloadByAcrossYears();
		EdocMarkReserveVO vo = markReservedVoMap.get(reserveId);
		if(vo == null){
			vo = historyMarkReservedVoMap.get(reserveId);
		}
		return vo;
	}
	...
	public void delCachEdocMarkReserve(Long delReservedId) {
		...
		if(historyMarkReservedVoMap.contains(delReservedId)){
			historyMarkReservedVoMap.remove(delReservedId);
		}
	}
				...
				if(Strings.isNotEmpty(reserveVO.getReserveNumberList())) {
					...
					for(EdocMarkReserveNumber reserveNumber : reserveVO.getReserveNumberList()) {
						...
						if(Strings.isNotEmpty(thisReservedIdList)) {
							if(!thisReservedIdList.contains(reserveNumber.getMarkNo())) {
								checkFlag = true;
							}
						} else {
							checkFlag = true;
	...
	public List<EdocMarkReserveVO> queryEdocMarkReserveList(Long markDefineId, Boolean yearEnabled, Integer queryNumber) throws BusinessException {
		return getReserveVoMap_Query(yearEnabled, queryNumber).get(markDefineId);
	}
	public Map<Long, List<EdocMarkReserveVO>> getAllEdocMarkReserveListMap() throws BusinessException {
		return getReserveVoMap_Query(Boolean.FALSE, -1);
	}
	private Map<Long, List<EdocMarkReserveVO>> getReserveVoMap_Query(Boolean yearEnable, Integer queryNumber) throws BusinessException {
		reloadByAcrossYears();
