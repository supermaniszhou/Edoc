 ...
 * @date 2013-4-25 
 */
public class EdocCheckPrivDelete implements CheckPrivUpdate {
	
	private static final Log LOGGER = CtpLogFactory.getLog(EdocCheckPrivDelete.class);
     ...
     */
    public  String processUpdate(Collection<V3xOrgMember> newMembers,
			Collection<V3xOrgMember> delMembers, String rolename, Long unitId)
			throws BusinessException  {   
    	
        String msg = null;
        LOGGER.info("单位收发员、部门收发员角色变动校验接口");
    	if(OrgConstants.Role_NAME.Accountexchange.name().equals(rolename) 
    	        || OrgConstants.Role_NAME.Departmentexchange.name().equals(rolename)){
    ...
    private String exchangeDelCheck(Collection<V3xOrgMember> newMembers,
        ...
        for(EdocSendRecord sendRec: sendList){
            //获得待交换的affair数据
            List<CtpAffair> affairList = getObjectIdListAffair(sendRec.getEdocId(), ApplicationCategoryEnum.exSend.getKey(),null);
        ...
        for(EdocRecieveRecord receiveRec: recieveList){
            //获得待交换的affair数据
            List<CtpAffair> affairList = getObjectIdListAffair(receiveRec.getEdocId(), ApplicationCategoryEnum.exSign.getKey(),null);
            if(affairList.size()==0){
                continue;
            }
        ...
        //删除的人员待交换数据物理删除
        if(delMembersAffairList.size()>0){
            for(Long c : delMembersAffairList){
    ...
    private List<CtpAffair> getObjectIdListAffair(Long objectId, int  exchangeType, Long memberId) throws BusinessException{
    	//待发送、待签收的数据
		...
		conditions.put("app", exchangeType);
    ...
    private void newPeopleAccept(List<Long> roleNewMemberIds,CtpAffair ctpAffair, List<CtpAffair> copyToNewMembersAffairList,Long subObjectId){
    	...
    	for(Long roleNewMemberId : roleNewMemberIds){
			...
			try {
				...
				c.setCreateDate(now);
				c.setReceiveTime(now);
				LOGGER.info(AppContext.currentUserName()+",变更角色生成affair数据 affairId:"+c.getId()+" memberId:"+c.getMemberId());
