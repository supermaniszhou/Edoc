...
public class EdocMarkReserveManagerImpl implements EdocMarkReserveManager {
	...
	public void reloadCache() {
		edocMarkReserveCacheManager.init();
	}
	...
	public void saveEdocMarkReserve(EdocMarkDefinition markDef, List<EdocMarkReserveVO> addReserveList, List<Long> delReservedIdList) throws BusinessException {
		...
		for(EdocMarkReserveVO delReserveVo : delReserveVoList) {
			for(EdocMarkReserveNumber reserveNumber : delReserveVo.getReserveNumberList()) {
				...
				if(usedMarkMap.containsKey(reserveNumber.getMarkNo().intValue())) {
					continue;
				}
	...
	public List<EdocMarkReserveNumber> findAllEdocMarkReserveNumberList() throws BusinessException {
		return findEdocMarkReserveNumberList(null, ReserveTypeEnum.reserve_all.getReserveType(), false);
	}
	...
	public List<EdocMarkReserveNumber> findEdocMarkReserveNumberList(EdocMarkDefinition markDef, Integer type) throws BusinessException {
		return findEdocMarkReserveNumberList(markDef, type, true);
	}
	private List<EdocMarkReserveNumber> findEdocMarkReserveNumberList(EdocMarkDefinition markDef, Integer type, Boolean onlyMy) throws BusinessException {
		...
		if(markDef == null) {
			if(onlyMy) {
				...
				reserveVoList = edocMarkReserveCacheManager.getAllReserveVoList();
			}
		} else {
	...
	public List<EdocMarkReserveVO> findEdocMarkReserveList(EdocMarkDefinition markDef, Integer queryNumber) throws BusinessException {
		List<EdocMarkReserveVO> reserveVoList = edocMarkReserveCacheManager.queryEdocMarkReserveList(markDef.getId(), markDef.getEdocMarkCategory().getYearEnabled(), queryNumber);
		if(Strings.isNotEmpty(reserveVoList)) {
	...
	public EdocMarkReserveVO getMarkReserveByFormat(EdocMarkDefinition markDef, Integer reserveYearNo, Integer markNumber) {
		...
		if(markNumber != null) {
			...
			reserveVO.setReserveLimitNo(Strings.getSafeLimitLengthString(reserveVO.getWordNo(), 15, "...")+reserveVO.getFormatA()+reserveVO.getYearNo()+reserveVO.getFormatB() + number + reserveVO.getFormatC());
			String wordNo = reserveVO.getWordNo()+reserveVO.getFormatA()+reserveVO.getYearNo()+reserveVO.getFormatB() + number + reserveVO.getFormatC();

	    	if(Strings.isNotBlank(wordNo)){
	    		wordNo = wordNo.replaceAll(String.valueOf((char)160), String.valueOf((char)32));
	    	}
			
			reserveVO.setReserveNo(wordNo);
		}
		return reserveVO;
	}
