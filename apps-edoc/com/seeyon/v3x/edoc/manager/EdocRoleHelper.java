 ...
 * public static String acountExchangeRoleName="account_exchange";
	...
	public static boolean isEdocCreateRole(Long accountId, Long userId, int edocType) throws BusinessException {
		...
		if (edocType == EdocEnum.edocType.sendEdoc.ordinal()) {
			isCreateRole = pcheck.checkByReourceCode("F07_sendManager", userId, accountId)
					&& (pcheck.checkByReourceCode("F07_sendNewEdoc", userId, accountId)
							|| pcheck.checkByReourceCode("F07_sendNewEdoc", userId, groupDomainId));
		}
		else if (edocType == EdocEnum.edocType.signReport.ordinal()) {
			isCreateRole = pcheck.checkByReourceCode("F07_signReport", userId, accountId)
					&& (pcheck.checkByReourceCode("F07_signNewEdoc", userId, accountId)
							|| pcheck.checkByReourceCode("F07_signNewEdoc", userId, groupDomainId));
		}
		else if (edocType == EdocEnum.edocType.recEdoc.ordinal()) {

			if (EdocHelper.isG6Version()) {
				// 是否有G6登记权
				isCreateRole = (pcheck.checkByReourceCode("F07_recListRegistering", userId, accountId)
						|| pcheck.checkByReourceCode("F07_recListRegistering", userId, groupDomainId));

			}
			else {
				isCreateRole = (pcheck.checkByReourceCode("F07_recRegister", userId, accountId)
						|| pcheck.checkByReourceCode("F07_recRegister", userId, groupDomainId));
			}
			...
			// 是否有G6分发权
			isCreateRole =  (pcheck.checkByReourceCode("F07_recListFenfaing", userId, accountId)
							|| pcheck.checkByReourceCode("F07_recListFenfaing", userId, groupDomainId));
		}
	...
	public static boolean isDepartmentExchange()  throws BusinessException
	{
		...
		*/
		...
		return orgManager.isRole(user.getId(), user.getDepartmentId(), departmentExchangeRoleName);
	}
	/**
	...
	public static boolean isDepartmentExchange(Long accountId,Long departmentId,Long userId)  throws BusinessException
	{
		...
		return orgManager.isRole(userId, departmentId, departmentExchangeRoleName);
	}
	/**
	...
	public static String getExchangeOgrNames(User user) throws BusinessException{
	    ...
	    boolean isAccountExcRole = EdocRoleHelper.isAccountExchange(user.getId());//是否为单位收发员
        if(isAccountExcRole){
