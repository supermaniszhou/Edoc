...
public class EdocRegisterDao extends BaseHibernateDao<EdocRegister> {
		...
		if (Strings.isNotBlank(conditionKey)) {
			if("recTime".equals(conditionKey)) {//签收时间
				...
				if (Strings.isNotBlank(textfield)) {//标题
					hsql.append(" and e.").append(conditionKey).append(" like :").append(conditionKey);
					parameterMap.put(conditionKey, "%" + SQLWildcardUtil.escape(textfield) + "%");
				}
			}			
   		}
	...
	public List<EdocRegister> findList(String registerUserIds, int state, int registerType, String condition, String[] value) {
		...
		if(condition!=null && "createPerson".equals(condition)) {
			if(value[0]!=null && !"".equals(value[0])) {
				hsql = new StringBuilder("select e from EdocSummary a ,EdocRegister e where a.id = e.distributeEdocId and e.state = ")
				          .append(state)
				          .append(" and a.createPerson like :createPerson ");
				parameterMap.put("createPerson", "%" + SQLWildcardUtil.escape(value[0]) + "%");
			}
		}
		//发起时间 
			...
			if(value[0]!=null && !"".equals(value[0])) {
				hsql.append(" and a.recTime >= :recTime0 ");
				parameterMap.put("recTime0", "%" + SQLWildcardUtil.escape(value[0]) + "%");
			} 
			if(value[1]!=null && !"".equals(value[1])) {
				hsql.append(" and a.recTime <= :recTime1 ");
				parameterMap.put("recTime1", "%" + SQLWildcardUtil.escape(value[1]) + "%");
			}
		}
		//签收时间  
		...
		//登记时间
			...
			hsql.append(" and e.").append(condition).append(" like :").append(condition);
			parameterMap.put(condition, "%" + SQLWildcardUtil.escape(value[0]) + "%");
		}
		if (registerType != EdocNavigationEnum.RegisterType.All.ordinal()) {
			hsql.append(" and e.registerType = :registerType ");
			parameterMap.put("registerType", registerType);
		...
		if(Strings.isNotBlank(registerUserIds)){
	          if(!edocAgent.isEmpty() && state != EdocNavigationEnum.RegisterState.DraftBox.ordinal()){//代理
				if (!agentToFlag) {
					hsql.append(" and (")
					    .append(" e.registerUserId in(:registerUserIds)");
					parameterMap.put("registerUserIds", registerUserIds);
					for(int i = 0 ; i < edocAgent.size() ; i++){
						AgentModel agent = edocAgent.get(i);
						hsql.append(" or e.registerUserId in (:agentToId"+i+")");
						parameterMap.put("agentToId"+i, agent.getAgentToId());
					}
					hsql.append(")");
				}else{
					hsql.append(" and e.registerUserId in (:registerUserIds)");
					parameterMap.put("registerUserIds", registerUserIds);
				}
	        }else{
	        	hsql.append(" and e.registerUserId in (:registerUserIds)");
