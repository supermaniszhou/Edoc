...
public class EdocOpinionDao extends BaseHibernateDao<EdocOpinion> {
    ...
    */
     ...
     * @param summaryId
     */
    public void deleteSenderOpinion(Long summaryId)
    {       
        String hsql="delete from EdocOpinion as eo where eo.edocSummary.id=? and eo.opinionType = "+EdocOpinion.OpinionType.senderOpinion.ordinal();
        super.bulkUpdate(hsql, null, summaryId);
    }
...
//  }
    ...
    */
    ...
    public List<EdocOpinion> findLastSortOpinionBySummaryIdAndPolicy(long summaryId, List<String> policy, String sortType, boolean isOnlyShowLast,boolean isBound,String optionType) {
        ...
        if(Strings.isNotEmpty(policy) && (policy.contains("inform") || policy.contains("zhihui"))){
        	policy.add("inform");
        	policy.add("zhihui");
        }
        ...
        while(it.hasNext()){
        	EdocOpinion opinion = (EdocOpinion)it.next();
        	if(opinion.getOpinionType().equals(EdocOpinion.OpinionType.senderOpinion.ordinal()) && opinion.getState() == 1){
        		it.remove();
        	}
        }
        return ls;
    ...
    public List<EdocOpinion> findEdocOpinionByAffairId(Long edocId,Long userId,String oplicy,List<Long> affairIds){
        ...
        List<Long>[] ids = Strings.splitList(affairIds, 999);
        for(int i = 0 ; i < ids.length ; i++){
        	StringBuffer hql = new StringBuffer("from EdocOpinion as eo where eo.edocSummary.id=:id and eo.createUserId=:userId and eo.policy=:policy");
        	hql.append(" and eo.affairId in (:affairIds" + i + ") ");
        	
        	Map<String,Object> map = new HashMap<String,Object>();
        	map.put("affairIds"+i, ids[i]);
        	map.put("id", edocId);
        	map.put("userId", userId);
        	map.put("policy", oplicy);
        	
        	List<EdocOpinion> ls=super.find(hql.toString(), map);
        	list.addAll(ls);
        }
        return list;
    }
...
//      super.getHibernateTemplate().update(hql, new Object[]{edocId,userid,oplicy,newstate,oldstate});
     ...
     * 获取公文意见条数
     */
    public int getOpinionCountByAffair(Long edocId, List<Integer> opinionTypes) {
    	StringBuffer hql = new StringBuffer();
    	Map<String,Object> map = new HashMap<String,Object>();
    	
    	hql.append("select count(*) from EdocOpinion where edocSummary.id = :edocId ");
    	map.put("edocId", edocId);
    	if (Strings.isNotEmpty(opinionTypes)) {
    		hql.append(" and opinionType in (:opinionTypes) ");
    		map.put("opinionTypes", opinionTypes);
    	}
    	
    	return DBAgent.count(hql.toString(), map);
    }
