...
//				 }
	...
	public void sendSupervisorMsgAndRecordAppLog(String caseId) {
		...
		try {
			...
			if(!edocSummary.getStartUserId().equals(user.getId())){
            	...
            	MessageContent content = new MessageContent("edoc.supervise.workflow.update", edocSummary.getSubject(), memname,app.ordinal()).setImportantLevel(importantLevel);
            	if (null != edocSummary.getTempleteId()) {
            		content.setTemplateId(edocSummary.getTempleteId());
            	}
	...
	@Override
	public List<WorkflowFormFieldBO> getWorkflowBranchFormFieldVOListById(Long id) {
	...
	@Override
	public Map<String, WorkflowFormFieldBO> getFormFieldsDefinition(String appName, String formAppId,
			List<String> fieldTypes, String externalType, List<String> tableTypes) {
		 List<WorkflowFormFieldBO> allEdocFields= edocFormManager.getAllElementsByEdocForm(Long.parseLong(formAppId));
		 Map<String, WorkflowFormFieldBO> fieldMap = new LinkedHashMap<String, WorkflowFormFieldBO>();
		 if(null!=allEdocFields && !allEdocFields.isEmpty()){
			 for (WorkflowFormFieldBO field : allEdocFields) {
	 ...
	 * @return
	 */
	private boolean getFieldTypeMatchResult(WorkflowFormFieldBO field, List<String> fieldTypes) {
	...
	@Override
	public boolean getSummaryHasAudit(String processId) {
		return false;
	}
