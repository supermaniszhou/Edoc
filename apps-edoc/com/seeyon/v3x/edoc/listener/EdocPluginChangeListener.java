 ...
 * @Date : 2017年3月16日
 */
public class EdocPluginChangeListener {
	private static Log LOG = CtpLogFactory.getLog(EdocPluginChangeListener.class);
				...
				if (Strings.isNotEmpty(allAccounts)) {
					...
					if (isCreate) {
						for (V3xOrgAccount account : allAccounts) {
							if (!account.isGroup()) {
								LOG.info("复制公文数据开始...");
								EdocHelper.generateZipperFleet(account.getId(),true);
								LOG.info("复制公文数据结束...");
							}
						}
					}
