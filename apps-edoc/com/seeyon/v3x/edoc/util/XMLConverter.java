...
//          判断是否为处理意见
            ...
            if(formElement.getElementId()==1){
                ...
                if(null!=edocSummary && null!=edocSummary.getSubject()){
                    ...
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getSubject()));
                }
            ...
            //------------changyi 增加附件和发文部门公文元素  end-------------------------
                ...
                if(null!=edocSummary && null!=edocSummary.getSendTo()){
                    ...
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getSendTo()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getSendTo2()){
                    ...
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getSendTo2()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getCopyTo()){
                    ...
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getCopyTo()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getReportTo()){
                    ...
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getReportTo()));
                }
            }
            ...
            //承办机构
                ...
                if(null!=edocSummary && null!=edocSummary.getText1()){
                	fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText1()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText2()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText2()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText3()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText3()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText4()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText4()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText5()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText5()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText6()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText6()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText7()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText7()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText8()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText8()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText9()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText9()));
                }
            }
                ...
                if(null!=edocSummary && null!=edocSummary.getText10()){
                    fieldValue = Strings.toHTML(Strings.toXmlStr(edocSummary.getText10()));
                }
            }
		...
		if (null != element && element.getStatus() != 0) {
			...
			if (null != metadataId) {
				...
				for (int x = 0; x < metaItem.size(); x++) {
					...
					String disable = "\" disable=\"false";
					if(metaDataItem.getState().intValue() == 0){
						disable = "\" disable=\"true";
					}
					if (Strings.isNotBlank(fieldValue) && (metaDataItem.getValue()).equals(fieldValue)) {
						strb.append(display).append(Strings.toXmlStr(Strings.toXmlStr(name))).append(displayValue)
								.append(metaDataItem.getValue()).append(disable).append("\" select=\"true\" ").append("/>");
						bool = true;
					} else {
						//当用户输入的内容中存在&信息时,toXmlStr一次转换不会转义到
						strb.append(display).append(Strings.toXmlStr(Strings.toXmlStr(name))).append(displayValue).append(metaDataItem.getValue()).append(disable).append(displayEnd);
					}
				}
				if (bool) {
    ...
    private void addDocMarkListList(StringBuffer sb,String fieldName,String fieldValue,String access,int markType,
		...
		try {
			...
			Long tempAccountId =  V3xOrgEntity.VIRTUAL_ACCOUNT_ID;


			StringBuilder deptIds = new StringBuilder(orgManager.getUserIDDomain(userId, tempAccountId,
					V3xOrgEntity.ORGENT_TYPE_DEPARTMENT, V3xOrgEntity.ORGENT_TYPE_ACCOUNT));
			if (isTemplete || (isNoShowOriginalDocMark && model != null)) {// 是单位管理员新建或者修改模板的话显示授权给这个单位下面部门的文号
																			// or
																			// 如果调用模板且模板绑定了文号，才需要查所有文号
			...
			if (summary.getId() != null && user.getAccountId().longValue() != user.getLoginAccount().longValue()) {
				...
				if (user.getAccountId().longValue() != user.getLoginAccount().longValue()) {
					...
					if (!(pv != null && "niwen".equals(pv.getName()))) {
						String myDeptIds = orgManager.getUserIDDomain(userId, user.getAccountId(),
								V3xOrgEntity.ORGENT_TYPE_DEPARTMENT, V3xOrgEntity.ORGENT_TYPE_ACCOUNT);
						deptIds.append(",");
		 ...
		 * 2模板与文号的关系在使用本单位授权模板、外单位授权模板逻辑一致 3当外部单位的模板使用的文号
		 * 没有授权给本单位的时候，在本单位调用该模板时，也要显示该文号
		 * 
		 */
		...
		if (model != null) {
			...
			List<EdocMarkAcl> acls = edocMarkAclDao.findEdocMarkAclByProperty("edocMarkDefinition.id",
					model.getMarkDefinitionId());
			// 当外单位的模板绑定的文号 授权了的
			if (isOuterTemplate && acls.size() != 0) {
				...
				if (count == 0 && Strings.isBlank(fieldValue)) {
					...
					list.add(model);
					// 当外部单位的模板使用的文号 没有授权给本单位的时候，在本单位调用该模板时，也要显示该文号(之前的判断
					// 没有包含这种情况，所以在补上)
		...
		// ----如果fieldValue不为空或不等于"",拼接成<Input display=...
		// "0|文号||0(Constants.EDOC_MARK_EDIT_NONE)".... select="true" />
		if (null != fieldValue && !"".equals(fieldValue)) {
			// 模板中使用的文号没有删除时
			if (!templateEdocMarkIsDelete) {
				// 切换文单时候，录入的临时文号需要解析
				try {
					fieldValueDocMarkModel = EdocMarkModel.parse(fieldValue);
					if(fieldValueDocMarkModel == null && templeteId != null){
						CtpTemplate template = templateManager.getCtpTemplate(templeteId);
						if(template != null){
							EdocSummary parentSummary = (EdocSummary) XMLCoder.decoder(template.getSummary());
							fieldValueDocMarkModel = EdocMarkModel.parse(parentSummary.getDocMark());
							isNoShowOriginalDocMark = true;
							
							sb.append(display);
							sb.append(Strings.toHTML(Strings.toXmlStr(fieldValue), false));
							sb.append(displayValue);
							sb.append("0|" + Strings.toHTML(Strings.toXmlStr(fieldValue), false) + "||"
									+ Constants.EDOC_MARK_EDIT_NONE);
				...
				if (fieldValueDocMarkModel == null && templeteId == null) {
					...
					if (isNoShowOriginalDocMark) {
						try {
							...
							EdocMarkDefinition markDef = edocMarkDefinitionManager
									.getMarkDefinition(fieldValueDocMarkModel.getMarkDefinitionId());
							// OA-33699 客户bug：授权给外单位来用的公文模板，外单位拟文人，拟文节点权限跟外单位的一致
							if (markDef != null) {
								Calendar cal = Calendar.getInstance();
								String yearNo = String.valueOf(cal.get(Calendar.YEAR));
								EdocMarkModel currentModel = edocMarkDefinitionManager.markDef2Mode(markDef, yearNo,
										null);
								String strTemp = currentModel.getMarkDefinitionId() + "|" + currentModel.getMark() + "|"
										+ currentModel.getCurrentNo() + "|" + Constants.EDOC_MARK_EDIT_SELECT_NEW;
								sb.append(display);

								String markWordNo = currentModel.getMark()
										+ EdocHelper.getEdocMarkDispalyName(user.getLoginAccount(), currentModel);
		...
		if (Strings.isBlank(fieldValue)) {
			...
			if ((model != null && validateEdocMark(fieldValue, fieldName, templeteId))
					|| isUseOuterAccountTemplateBindingEdocMark) {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				if (null != markModel) {
					...
					strTemp = definitionId + "|" + markModel.getMark() + "|" + markModel.getCurrentNo() + "|"
							+ Constants.EDOC_MARK_EDIT_SELECT_NEW;
					...
					if (isTemplete) {
						...
						if (fieldValueDocMarkModel != null
								&& definitionId.equals(fieldValueDocMarkModel.getMarkDefinitionId())) {
							// 修改模板的时候前端可能已经调用过这个模板来，流水号已经增加了，为了达到修改选中的效果，将下拉的流水号的当前值设置为模板保存当时的值。
							templateDocMarkNo = fieldValueDocMarkModel.getCurrentNo();
						}
						String markWordNo = markModel.getWordNo()
								+ EdocHelper.getEdocMarkDispalyName(user.getLoginAccount(), markModel);
						sb.append(Strings.toHTML(Strings.toXmlStr(markWordNo), false));
						sb.append(displayValue);
						strTemp = definitionId + "|" + markModel.getWordNo() + "|" + templateDocMarkNo + "|"
								+ Constants.EDOC_MARK_EDIT_SELECT_NEW;
						sb.append(Strings.toHTML(Strings.toXmlStr(strTemp), false));
					} else {
						String markWordNo = markModel.getMark()
								+ EdocHelper.getEdocMarkDispalyName(user.getLoginAccount(), markModel);
						sb.append(Strings.toHTML(Strings.toXmlStr(markWordNo), false));
						sb.append(displayValue);
						sb.append(Strings.toHTML(Strings.toXmlStr(strTemp), false));
