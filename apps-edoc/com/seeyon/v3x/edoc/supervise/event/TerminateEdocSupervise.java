	...
	public void execute(JobExecutionContext datamap) throws JobExecutionException{
		...
		try{
	    	...
	    	try{
	    		...
	    		MessageContent content = new MessageContent(key, subject,appEnum.getKey());
	    		if (null != edocSummary && null != edocSummary.getTempleteId()) {
	    			content.setTemplateId(edocSummary.getTempleteId());
	    		}
	    		userMessageManager.sendSystemMessage(content, appEnum, detail.getSenderId(), receivers,EdocMessageFilterParamEnum.supervise.key);
