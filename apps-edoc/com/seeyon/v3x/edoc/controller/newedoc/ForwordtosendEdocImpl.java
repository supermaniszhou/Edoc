      ...
      //  summary.setFormId(defaultEdocForm.getId());//元素绑定表单
        ...
        if(body !=null){
            ...
            if(Strings.isNotBlank(content)){
            	V3XFile file = fileManager.getV3XFile(Long.valueOf(content));
            	if(file != null){
             ...
             if("1".equals(checkOption)){
                ...
                if((Constants.EDITOR_TYPE_OFFICE_EXCEL.equals(body.getContentType())
                    ...
                    try { 
                        ...
                        //GOV-4751 收文转发文时选择将收文的正文作为新发文的附件，结果查看新发文的正文，也显示成收文正文的内容了（正常应该显示为空，原收文的正文以附件形式显示）。
                        V3XFile f = fileManager.save(in, ApplicationCategoryEnum.edoc, attSubject + EdocUtil.getOfficeFileExt(body.getContentType()), fileDate, false);
                    ...
                    String srcPath=fileManager.getFolder(fileDate, true) + separator+ String.valueOf(body.getContent());
                    File srcFile = new File(srcPath);
                    if(srcFile.exists() && srcFile.isFile()){
                    	InputStream in = new FileInputStream(srcFile) ;
                    	//GOV-4751 收文转发文时选择将收文的正文作为新发文的附件，结果查看新发文的正文，也显示成收文正文的内容了（正常应该显示为空，原收文的正文以附件形式显示）。
                    	V3XFile f = fileManager.save(in, ApplicationCategoryEnum.edoc, attSubject + ".pdf", fileDate, false);
                    	atts.add(new Attachment(f, ApplicationCategoryEnum.edoc, com.seeyon.ctp.common.filemanager.Constants.ATTACHMENT_TYPE.FILE));
                    	try {
                        	in.close();
                        } catch (Exception e) {}
                ...
                {//html正文，先生成文件         
                    //GOV-4751 收文转发文时选择将收文的正文作为新发文的附件，结果查看新发文的正文，也显示成收文正文的内容了（正常应该显示为空，原收文的正文以附件形式显示）。
                    V3XFile f =fileManager.save(body.getContent()==null?"":body.getContent(), ApplicationCategoryEnum.edoc, attSubject+".htm", fileDate, false);             
                    atts.add(new Attachment(f, ApplicationCategoryEnum.edoc, com.seeyon.ctp.common.filemanager.Constants.ATTACHMENT_TYPE.FILE));
                }
            }//将原有正文生成新公文附件 
