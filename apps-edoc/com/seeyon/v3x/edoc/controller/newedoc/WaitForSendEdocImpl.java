...
public class WaitForSendEdocImpl extends NewEdocHandle {
	...
	public void createEdocSummary(HttpServletRequest request, ModelAndView modelAndView) throws Exception {
		...
		try {
			...
			isSpecialSteped= affair != null &&  affair.getSubState() != null && affair.getSubState().equals(SubStateEnum.col_pending_specialBacked.getKey());
		...
		if (templete == null && isGroupVer) {// 如果不是集团版本，就不用判断文单的授权了
			...
			List<V3xOrgAccount> childrenAccount = new ArrayList<V3xOrgAccount>();
			for (EdocFormAcl ec : formAcls) {
				if(user.getLoginAccount().equals(ec.getDomainId())){
					containFlag = true;
					break;
				}
				childrenAccount = orgManager.getAllParentAccount(user.getLoginAccount());
				for(V3xOrgAccount account : childrenAccount){
					if(account.getId().equals(ec.getDomainId())){
						containFlag = true;
						break;
					}
				}
