...
public class EdocElementController extends BaseController{
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(StringUtils.isNotBlank(condition)){
			...
			list1 = edocElementManager.getEdocElementsByContidion(condition,textfield,statusSelect,0);
			Pagination.setRowCount(list1.size());
		}
		//没有条件返回的查询
		else{
	...
	public ModelAndView update(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotBlank(metadataId)) {
			...
			if(ctpEnum == null){
	    		...
	    		pw.println("<script>");
	    		pw.println("alert(\""+outMsg+Strings.toHTML(metadataName)+stratMsg+"\");");
