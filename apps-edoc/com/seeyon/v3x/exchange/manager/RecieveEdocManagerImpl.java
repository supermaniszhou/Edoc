...
public class RecieveEdocManagerImpl implements RecieveEdocManager {
	...
	public void create(EdocSendRecord edocSendRecord,
		...
		for(V3xOrgMember m:member){
			...
			MessageContent content = new MessageContent(key,affair.getSubject(),userName);
			if (null != affair.getTempleteId()) {
				content.setTemplateId(affair.getTempleteId());
			}
			userMessageManager.sendSystemMessage(content, ApplicationCategoryEnum.exSign, user.getId(), receiver_a,EdocMessageFilterParamEnum.exchange.key);	
			}
	}
		...
		for(V3xOrgMember m:member){
			...
			if(agentToId !=null){//当前处理人是代理人
				if(agentMemberId!=null){//当前查找出来的人还设置了代理人
					MessageContent content = new MessageContent(key,affair.getSubject(),agentToName).add("edoc.agent.deal", user.getName()).add("col.agent");
					if (null != affair.getTempleteId()) {
						content.setTemplateId(affair.getTempleteId());
					}
					userMessageManager.sendSystemMessage(content, ApplicationCategoryEnum.exSign, agentToId, agentReceiver,EdocMessageFilterParamEnum.exchange.key);
				}
				MessageContent content1 = new MessageContent(key,affair.getSubject(),agentToName).add("edoc.agent.deal", user.getName());
				if (null != affair.getTempleteId()) {
					content1.setTemplateId(affair.getTempleteId());
				}
	        	userMessageManager.sendSystemMessage(content1, ApplicationCategoryEnum.exSign, agentToId, receiver_a,EdocMessageFilterParamEnum.exchange.key);
	        }else{
	        	if(agentMemberId!=null){//当前查找出来的人还设置了代理人
	        		MessageContent content = new MessageContent(key,affair.getSubject(),userName).add("col.agent");
	        		if (null != affair.getTempleteId()) {
						content.setTemplateId(affair.getTempleteId());
					}
	        		userMessageManager.sendSystemMessage(content, ApplicationCategoryEnum.exSign, user.getId(), agentReceiver,EdocMessageFilterParamEnum.exchange.key);
	        	}
	        	MessageContent content1 = new MessageContent(key,affair.getSubject(),userName);
	        	if (null != affair.getTempleteId()) {
					content1.setTemplateId(affair.getTempleteId());
				}
	        	userMessageManager.sendSystemMessage(content1, ApplicationCategoryEnum.exSign, user.getId(), receiver_a,EdocMessageFilterParamEnum.exchange.key);
			}
		}
