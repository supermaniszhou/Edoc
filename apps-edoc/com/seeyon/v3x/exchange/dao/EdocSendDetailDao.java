...
public class EdocSendDetailDao extends BaseHibernateDao<EdocSendDetail>{
	...
	public List<Object[]> getMarkList1(Long accountId,Long depId){
		String sql =  "select e.id,e.wordNo,e.expression,c.yearEnabled,c.maxNo,e.length,c.currentNo from EdocMarkDefinition as e,EdocMarkCategory as c,EdocMarkAcl as a  where e.id=a.edocMarkDefinition.id and c.id = e.edocMarkCategory.id and e.domainId = ? and e.markType=2 and(a.deptId=? or a.deptId=?) and e.status!=?";
	...
	public List<Object[]> getMarkList2(Long accountId,Long depId){
		String sql =  "select e.id,e.wordNo,e.expression,c.yearEnabled,c.maxNo,e.length,c.currentNo from EdocMarkDefinition as e,EdocMarkAcl as a,EdocMarkCategory as c  where e.id=a.edocMarkDefinition.id and c.id = e.edocMarkCategory.id and (a.deptId = ? or a.deptId = ?) and e.markType=2 and e.status!=?";
	...
	public List<Object[]> getMarkList(List<Long> deptIdList) {
		...
		buffer.append(" where e.id=a.edocMarkDefinition.id and c.id = e.edocMarkCategory.id");
